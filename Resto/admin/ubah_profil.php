...
if (isset($_POST['ChangeProfile'])) {
  ...
  $Qry = "UPDATE admin SET nama_admin =?, email_admin =? WHERE id_admin =?";
  $postStmt = $mysqli->prepare($Qry);
  //bind paramaters
  $rc = $postStmt->bind_param('sss', $nama_admin, $email_admin, $id_admin);
  $postStmt->execute();
...
if (isset($_POST['changePassword'])) {
  ...
  if (!$error) {
    ...
    if (mysqli_num_rows($res) > 0) {
      ...
      if ($old_password != $row['password_admin']) {
        ...
        $query = "UPDATE admin SET  password_admin =? WHERE id_admin =?";
        $stmt = $mysqli->prepare($query);
        //bind paramaters
        $rc = $stmt->bind_param('ss', $new_password, $id_admin);
        $stmt->execute();

        //declare a varible which will be passed to alert function
        if ($stmt) {
          $success = "Password Terubah" && header("refresh:1; url=dashboard.php");
        } else {
...
<body>
  ...
  <div class="main-content">
    ...
    ?>
      <!-- Header -->
      <div class="header pb-8 pt-5 pt-lg-8 d-flex align-items-center" style="min-height: 600px; background-image: url(assets/img/theme/bg.jpg); background-size: cover; background-position: center top;">
        <!-- Mask -->
        ...
        <div class="container-fluid d-flex align-items-center">
          <div class="row">
            <div class="col-lg-7 col-md-10">
              <h1 class="display-2 text-white">Halo <?php echo $admin->nama_admin; ?></h1>
            ...
            <div class="card card-profile shadow">
              ...
              <div class="card-body pt-0 pt-md-4">
                ...
                <div class="text-center">
                  <h3>
                    <?php echo $admin->nama_admin; ?></span>
                  </h3>
                  <div class="h5 font-weight-300">
                    <i class="ni location_pin mr-2"></i><?php echo $admin->email_admin; ?>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <div class="row">
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="text" name="nama_admin" value="<?php echo $admin->nama_admin; ?>" id="input-username" class="form-control form-control-alternative" ">
                      </div>
                    </div>
                    <div class=" col-lg-6">
                          <div class="form-group">
                            <label class="form-control-label" for="input-email">Alamat Email</label>
                            <input type="email" id="input-email" value="<?php echo $admin->email_admin; ?>" name="email_admin" class="form-control form-control-alternative">
                          </div>
                        </div>

                        <div class="col-lg-12">
                          <div class="form-group">
                            <input type="submit" id="input-email" name="ChangeProfile" class="btn btn-success form-control-alternative" value="Simpan"">
                      </div>
                    </div>
                  </div>
                </div>
